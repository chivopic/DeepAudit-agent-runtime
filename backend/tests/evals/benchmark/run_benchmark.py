...
async def react_preconditions() -> list[str]:
    ...
    benchmark: it would "prove" the new engine wins. So the runner refuses to
    present a degraded ReAct run as a comparison.
    """
    missing: list[str] = []

    from app.core.config import settings

    # RAG retrieval needs an embedding provider; without it the analysis agent
    # loses code search and falls back to reading files blind.
    if not (
        getattr(settings, "EMBEDDING_API_KEY", None)
        or getattr(settings, "OPENAI_API_KEY", None)
    ):
        missing.append("embeddings (no EMBEDDING_API_KEY/OPENAI_API_KEY) — RAG disabled")

    # Semgrep, Bandit, Gitleaks and friends all run inside the sandbox image.
    try:
        import docker
        ...
        missing.append(f"docker unavailable ({exc}) — external scanners disabled")
...
async def run_react(root: str) -> EngineScore:
    degraded = await react_preconditions()
...
async def main() -> int:
    ...
    for name in names:
        ...
        results.append(await runner(root))
